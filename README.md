...
The agent, unprompted, with no one asking, notices the payment land,
checks the coin it was paid in for scam risk, and only then tells the
owner *"PAYMENT VERIFIED — safe to trust."* If the payment arrived in a
...
The actual showcase write-up lives at [`SHOWCASE.md`](./SHOWCASE.md); this
README is the deeper technical reference behind it. Three focused docs
go deeper still on specific topics covered below:
...
## `solana-core`: shared infrastructure worth its own look

The bounty brief explicitly calls this out as its own valid contribution:
*"a clean wasm32-wasip2 Solana core crate... proven by a real plugin and
a showcase demonstrating it, competes for a top prize."* That's exactly
what `solana-core` is here, not an afterthought shared folder, a real,
independently tested toolbox:

- **`pubkey.rs`**: base58 address parsing, the fail-closed foundation
  every plugin's argument validation rests on.
- **`rpc.rs`**: a thin, transport-agnostic JSON-RPC request/response
  layer (the actual network call is made by each plugin's own wasm
  shim via `waki`; this module just builds and parses the JSON).
- **`token.rs`**: SPL Token / Token-2022 mint account TLV parsing,
  including the transfer-hook, transfer-fee, and permanent-delegate
  extensions `token-risk-check` screens for.
- **`risk.rs`**: the shared scam-risk heuristic, `assess()`, the exact
  function both `token-risk-check` and `payment-watch` call, so they
  can never disagree about what counts as safe.
- **`dex.rs`**: optional liquidity-pool depth lookup (Dexscreener),
  fail-open by design so a missing or unreachable answer can never
  downgrade an already-clean verdict or launder an already-red one.
- Durable-nonce and transaction-building helpers, added when
  `spl-transfer-build` needed them, immediately reusable by any future
  plugin in this repo.

45 tests, host-only, zero wasm dependency, zero live network access
required. Vendored into all five plugins (`tools/sync_solana_core.py`
keeps every copy byte-identical to the canonical source), so it's
proven by five real, shipped plugins, not one.
...
was necessary in isolation. Its **swap-preparation** upgrade is a
different story: the bounded swap amount and slippage cap are enforced
in code, and `certify_swap` independently re-derives the customer's own
associated token account before ever returning a transaction, guarantees
a skill genuinely cannot make. What *does* need compiled code, and is
the actual reason `solana-pay-request` and `payment-watch` exist as
plugins rather than skills, is the fused, unconditional risk check
described above: an LLM-driven skill cannot *guarantee* a second check
always runs; a direct Rust function call can, and does.

**Chain-native spend caps (Subscriptions & Allowances)**: not
applicable here, since nothing in this repo signs or submits a
transaction, there is no T2 surface for an on-chain spend limit to
protect. Noted for completeness, not glossed over.
...
- **Blockhash expiry (trap #1).** `spl-transfer-build` supports durable
  nonce accounts, the real fix, not a workaround. All three processes
...
- **The wasm dependency wall (trap #2).** Confirmed by build, not by
  ...
  assumed. Full write-up in [`WASM_NOTES.md`](./WASM_NOTES.md).
...
Full step-by-step: [`REPRODUCIBILITY.md`](./REPRODUCIBILITY.md). Short
...
the actual running bot resolved `bonfida.sol` to
