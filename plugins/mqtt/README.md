# mqtt - ZeroClaw channel plugin source

This directory is the Phase 4 migration landing point for the built-in
`mqtt` channel. The manifest declares `provides = "mqtt"`, so
when it becomes publishable it will read the existing `[channels.mqtt.*]`
...
MQTT requires host-managed broker connectivity or a future WASI TCP/TLS capability before this source-only plugin can publish.
