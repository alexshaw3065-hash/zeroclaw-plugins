# twitch - ZeroClaw channel plugin source

This directory is the Phase 4 migration landing point for the built-in
`twitch` channel. The manifest declares `provides = "twitch"`, so
when it becomes publishable it will read the existing `[channels.twitch.*]`
...
Twitch chat requires host-managed IRC/TLS socket support before this source-only plugin can publish.
