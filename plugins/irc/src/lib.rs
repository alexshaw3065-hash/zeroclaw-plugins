//! A ZeroClaw WIT **channel** plugin source scaffold: IRC.
//!
//! This is the Phase 4 migration landing point for the built-in `irc`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "irc"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::irc::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
