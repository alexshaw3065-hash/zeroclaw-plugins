# email - ZeroClaw channel plugin source

This directory is the Phase 4 migration landing point for the built-in
`email` channel. The manifest declares `provides = "email"`, so
when it becomes publishable it will read the existing `[channels.email.*]`
...
Email requires host-managed IMAP/SMTP/TLS or a future socket/mail capability before this source-only plugin can publish.
