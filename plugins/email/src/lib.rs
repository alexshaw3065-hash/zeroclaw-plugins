//! A ZeroClaw WIT **channel** plugin source scaffold: Email.
//!
//! This is the Phase 4 migration landing point for the built-in `email`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "email"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::email::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
