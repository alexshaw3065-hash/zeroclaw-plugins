//! A ZeroClaw WIT **channel** plugin source scaffold: Linq.
//!
//! This is the Phase 4 migration landing point for the built-in `linq`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "linq"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::linq::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
