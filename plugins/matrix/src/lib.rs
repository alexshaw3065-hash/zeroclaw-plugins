//! A ZeroClaw WIT **channel** plugin source scaffold: Matrix.
//!
//! This is the Phase 4 migration landing point for the built-in `matrix`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "matrix"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::matrix::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
