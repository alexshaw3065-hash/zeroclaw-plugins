//! A ZeroClaw WIT **channel** plugin source scaffold: ClawdTalk.
//!
//! This is the Phase 4 migration landing point for the built-in `clawdtalk`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "clawdtalk"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::clawdtalk::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
