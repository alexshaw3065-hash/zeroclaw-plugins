# amqp - ZeroClaw channel plugin source

This directory is the Phase 4 migration landing point for the built-in
`amqp` channel. The manifest declares `provides = "amqp"`, so
when it becomes publishable it will read the existing `[channels.amqp.*]`
...
AMQP requires a host-managed AMQP client or a future WASI TCP/TLS capability before this source-only plugin can publish.
