//! A ZeroClaw WIT **channel** plugin source scaffold: AMQP.
//!
//! This is the Phase 4 migration landing point for the built-in `amqp`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "amqp"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::amqp::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
