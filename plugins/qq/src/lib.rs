//! A ZeroClaw WIT **channel** plugin source scaffold: QQ Official.
//!
//! This is the Phase 4 migration landing point for the built-in `qq`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "qq"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::qq::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
