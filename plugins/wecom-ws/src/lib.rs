//! A ZeroClaw WIT **channel** plugin source scaffold: WeCom WebSocket.
//!
//! This is the Phase 4 migration landing point for the built-in `wecom_ws`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "wecom_ws"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::wecom_ws::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
