# signal - ZeroClaw channel plugin source

This directory is the Phase 4 migration landing point for the built-in
`signal` channel. The manifest declares `provides = "signal"`, so
when it becomes publishable it will read the existing `[channels.signal.*]`
...
Signal depends on the host-managed signal-cli/native service boundary before this source-only plugin can publish.
