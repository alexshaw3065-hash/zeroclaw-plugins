//! A ZeroClaw WIT **channel** plugin source scaffold: Signal.
//!
//! This is the Phase 4 migration landing point for the built-in `signal`
//! channel. It compiles as a channel plugin and mirrors the existing channel
//! config via `provides = "signal"`, but remains `registry = false` until
...
mod component {
    ...
    use crate::signal::{send_unavailable, SourceOnlyConfig, CHANNEL, PLUGIN_NAME};
